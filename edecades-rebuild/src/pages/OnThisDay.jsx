  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-12">
      ...
      {todayEvents.length > 0 ? (
          ...
          <div className="space-y-4">
            {todayEvents.map((e, i) => (
              <button key={i} onClick={() => setModal(e)} className="w-full text-left">
                <div className="bg-charcoal border border-rose-gold/40 rounded-2xl p-6 hover:border-rose-gold transition-all hover:-translate-y-0.5 shadow-lg shadow-rose-gold/5">
        ...
        <section className="mb-12">
          <div className="bg-charcoal border border-white/10 rounded-2xl p-8 text-center">
            ...
            <p className="text-gray-400 text-sm">Browse the full historical archive below — history is always happening.</p>
          </div>
        </section>
      )}
      ...
      {upcomingEvents.length > 0 && (
        <section className="mb-12">
          <h2 className="font-retro text-xl text-white mb-4">🗓️ Coming Up This Year</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            {upcomingEvents.map((e, i) => (
              <button key={i} onClick={() => setModal(e)} className="text-left">
                <div className="bg-charcoal rounded-xl p-4 border border-white/10 hover:border-rose-gold/30 transition-all">
      ...
      <div className="space-y-3">
        {allFiltered.map((e, i) => (
          <button key={i} onClick={() => setModal(e)} className="w-full text-left">
            <div className={`rounded-xl p-4 border transition-all hover:border-rose-gold/30 ${
              ...
              <h3 className="text-white text-sm font-medium mt-1">{e.title}</h3>
            </div>
          </button>
        ))}
      </div>

      {/* Lightbox modal — amber themed */}
      {modal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm"
          onClick={() => setModal(null)}>
          <div className="bg-charcoal-dark border border-amber-600/50 rounded-2xl max-w-lg w-full p-7 shadow-2xl shadow-amber-900/30 relative"
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(modal.title + ' ' + modal.year + ' history')}`}
                target="_blank" rel="noopener noreferrer"
