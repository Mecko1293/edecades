...
export default function YouTubeDecades() {
  ...
  const show = filter === "All" ? DECADES : DECADES.filter(d => d.label === filter);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#1a1f2e", color: "#f0ece4", fontFamily: "Georgia, serif", paddingBottom: "60px" }}>
      ...
      <div style={{ background: "#2a3040", padding: "32px 20px", textAlign: "center", borderBottom: "3px solid #d4956e" }}>
        ...
        <p style={{ color: "#999", margin: 0 }}>Decade-tagged YouTube content</p>
      </div>
            ...
            <div key={d.label} style={{ marginBottom: "40px" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "14px" }}>
                {items.map(item => {
                  ...
                  const thumb = s.thumbnails.high ? s.thumbnails.high.url : s.thumbnails.medium ? s.thumbnails.medium.url : s.thumbnails.default.url;
                  return (
                    <div key={vid} onClick={() => setModal({ vid, title: s.title, color: d.color })}
                      ...
                      <div style={{ position: "relative" }}>
                        ...
                        <div style={{ position: "absolute", top: "8px", left: "8px", background: d.color, color: "#fff", fontSize: "0.7rem", fontWeight: "bold", padding: "2px 8px", borderRadius: "10px" }}>{d.label}</div>
                        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <div style={{ width: "44px", height: "44px", background: "rgba(255,0,0,0.85)", borderRadius: "50%", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", fontSize: "18px" }}>▶</div>
                        </div>
                      </div>
                      <div style={{ padding: "10px" }}>
                        <p style={{ margin: "0 0 4px", fontSize: "0.85rem", color: "#eee", lineHeight: "1.3" }}>{s.title.length > 60 ? s.title.substring(0, 60) + "..." : s.title}</p>
      ...
      {modal && (
        <div onClick={() => setModal(null)} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.9)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 9999, padding: "16px" }}>
          <div onClick={e => e.stopPropagation()} style={{ width: "100%", maxWidth: "780px", background: "#1a1f2e", borderRadius: "12px", overflow: "hidden", border: "2px solid " + modal.color }}>
            <div style={{ position: "relative", paddingBottom: "56.25%", height: 0 }}>
              <iframe src={"https://www.youtube.com/embed/" + modal.vid + "?autoplay=1"}
            ...
            <div style={{ padding: "14px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setModal(null)} style={{ background: "transparent", border: "1px solid #555", color: "#aaa", padding: "5px 12px", borderRadius: "6px", cursor: "pointer" }}>✕</button>
            </div>
          </div>
        </div>
      )}
